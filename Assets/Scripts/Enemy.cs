...
public class Enemy : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        _ghostLayer = this.gameObject.layer;
        Physics2D.IgnoreLayerCollision(this.gameObject.layer, _ghostLayer, true);
        Physics2D.IgnoreLayerCollision(this.gameObject.layer, _stunnedLayer, true);
    }
    ...
    void OnTriggerEnter2D(Collider2D collision)
    {
        if ((collision.tag == "Player") && !isStunned)
        {
            ...
            // if (player.playerCanMove || player.isElectric)
            // {
    ...
    void SpawnCoin()
    {
        ...
        {
            GameObject spawnCoin = Instantiate(coin, transform.position, Quaternion.identity);
            spawnCoin.GetComponent<SpawnCoin>().SetJump();
        }
    }
}
